...
#ifndef INCLUDE_vnx_RequestQueue_ASYNC_CLIENT_HXX_
#define INCLUDE_vnx_RequestQueue_ASYNC_CLIENT_HXX_
...
class RequestQueueAsyncClient : public vnx::AsyncClient {
public:
	RequestQueueAsyncClient(const std::string& service_name);
	
	RequestQueueAsyncClient(vnx::Hash64 service_addr);
...
private:
	...
	std::unordered_map<uint64_t, std::pair<std::function<void()>, std::function<void(const vnx::exception&)>>> vnx_queue_purge;
	std::unordered_map<uint64_t, std::pair<std::function<void()>, std::function<void(const vnx::exception&)>>> vnx_queue_clear;
...
#endif // INCLUDE_vnx_RequestQueue_ASYNC_CLIENT_HXX_
